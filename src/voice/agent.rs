...
pub async fn agent() {
    ...
    let llm_agent = client
        .agent("llama3.2")
        .preamble("You are a helpful voice assistant. Keep responses concise and conversational.")
        .build();
    ...
    loop {
        ...
        while mic_buffer.len() >= MIC_CHUNK_SIZE {
            ...
            // Once VAD has gone quiet and we have enough audio, process it
            if was_speaking && current_speech.len() > SAMPLE_RATE as usize {
                ...
                if response_text.is_empty() {
                    println!("[agent] (empty LLM response, skipping TTS)");
                    println!("\n[agent] Listening...");
                    continue;
                }
